...
def parse_bad_dates(raw, fmt=default_strptime_fmt):
    """
...
class ProxyList(list, BatchBase):
...
class Values(ProxyList):
    ...
    def __init__(self, values):
        ...
        not_empty = takewhile(bool, stripped)
        super().__init__(float(value) for value in not_empty)
...
class Parameter(BatchBase):
    ...
    def __init__(self, header, times, values):
        ...
        self._times.Parent = self
        self._values.Parent = self

        if len(self._times) != len(self._values):
            raise BatchError("Malformed batch data.")
    ...
    def __getitem__(self, item):
        """
        @param item: int | slice
        @type item: int | slice
        @return:
        @rtype: (datetime, float) | (list[datetime], list[float])
        """
        return self._times[item], self._values[item]
