...
debug_flags = "-alwaysOnTop -nosession"
        ...
        def copy_widget(widget, new_master):
            ...
            try:
                ...
                if is_good_tk_master(widget) and 'text' in widget.config().keys():
                    config_info['text'] = config_info['text'] + " copy"
            except:
...
def __make_cell_coords(self, master, widget_type=VC_COORD_WIDGET):
    ...
    if widget_type == 'Label':
        ...
        def make_coord(row, col, master=master):
            return ttk.Button(master, font=font, text="(r%d,c%d)" % (row,col), width=0, command=lambda: print("(r%d,c%d)" % (row,col)))
        ...
        def make_coord(row, col, master=master):
            ...
            label_widget = ttk.Label(widget, font=font, text="(r%d,c%d)" % (row,col),anchor=tk.CENTER)
            label_widget.grid()
            return widget
        ...
        def make_coord(row, col, master=master):
            widget = tk.Canvas(master, width=0, height=0, relief=VC_COORD_RELIEF, borderwidth=0)
    ...
    for k,v in cls.__dict__.items():
        ...
        if (not k.startswith("__")) and (not k.endswith("__")): 
            ...
            if isinstance(v, FunctionType):
                setattr(cls, k, traceFuncCalls(v, annotation=cls_msg.format(k), functype="unbdfn"))
                
            elif isinstance(v, MethodType):
                setattr(cls, k, traceFuncCalls(v, annotation=cls_msg.format(k), functype="bdfn"))
